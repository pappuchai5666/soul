...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @@PRINCE_OP5! /get_id*", parse_mode='Markdown')
        return
    message = (
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @@PRINCE_OP5!*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
