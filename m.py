...
bot = telebot.TeleBot('7635258880:AAFfn3vPkkbSCKzIrFUz_qkw5-ymX5PYRl0')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "You have not purchased yet purchase now from:- @PRINCE_OP5."
...
✅ Usage: /remove <userid>'''
        ...
        response = "YOU HAVE NOT PRCHASE THIS BOT PLEASE CONTACT :- @TRUSTVIP_MOD0."
...
def show_recent_logs(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝙏𝙝𝙞𝙨 𝘽𝙤𝙩 𝙞𝙨 𝙤𝙣𝙡𝙮 𝙛𝙤𝙧 𝙥𝙖𝙞𝙙 𝙪𝙨𝙚𝙧𝙨 𝙗𝙪𝙮 𝙣𝙤𝙬 𝙛𝙧𝙤𝙢 - @TRUSTVIP_MOD0 "
        bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = ("  🚧   अरे चोमू खरीद लो लौडे   🤣 :- @TRUSTVIP_MOD0   ")
...
Official Channel :- https://t.me/+OcyU6mJB3ys5NmY1
'''
...
def welcome_start(message):
    ...
    response = f'''❄️𝗪𝗘𝗟𝗖𝗢𝗠𝗘 {user_name} 𝗧𝗢 𝗣𝗥𝗘𝗠𝗜𝗨𝗠 𝗗𝗗𝗢'𝗦 𝗕𝗢𝗧 𝗧𝗛𝗜𝗦 𝗜𝗦 𝗛𝗜𝗚𝗛 𝗤𝗨𝗔𝗟𝗜𝗧𝗬 𝗦𝗘𝗥𝗩𝗘𝗥 𝗕𝗔𝗦𝗘𝗗 𝗗𝗗𝗢𝗦 𝗕𝗢𝗧 .
...
Group, Brother Only 1 Plan Is Powerfull Then Any Other Ddos !!:
...
♨️ YOUR OWN DDOS BOT 1.5k RS DISCOUNT AVAILABLE 

DM :- @TRUSTVIP_MOD0
DDOS BOT :- @TRUSTVIPKADDOS_BOT
'''
    bot.reply_to(message, response)
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "⚠️ 𝙈𝙚𝙨𝙨𝙖𝙜𝙚 𝙩𝙤 𝙖𝙡𝙡 𝙪𝙨𝙚𝙧𝙨 𝙛𝙧𝙤𝙢 TRUSTVIP_MOD0:\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
